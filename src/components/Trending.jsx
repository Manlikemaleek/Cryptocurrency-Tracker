  ...
  return (
    <div className='rounded-div my-12 py-8 text-primary'>
       ...
       <div className='grid md:grid-cols-2 lg:grid-cols-3 gap-4'>
         {trending.map((coins, index) => (
           <div key={index} className='rounded-div flex justify-between p-4 hover:scale-105 ease-in-out duration-300'>
             <div className='flex w-full items-center justify-between'>
               <div className='flex'>
                 <img className='mr-4 rounded-full' src={coins.item.small} alt="/" />
                 <div>
                   <p className='font-bold'>{coins.item.name}</p>
               ...
               <div className='flex items-center'>
                 ...
                 <p className='mr-2'>{coins.item.price_btc.toFixed(7)}</p>
               </div>
             </div>
           </div>
         ))}
